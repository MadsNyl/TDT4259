...
# Read the csv file
df = pd.read_csv(f"{WASHED_DATA_PATH}washed_filter_out_women_{NOT_MURDER_VICTIM_AGE_SEX}")
...
sum_by_year.to_csv(f"{SUM_DATA}sum_for_each_year_{NOT_MURDER_VICTIM_AGE_SEX}", index=False)